...
WeightPlan bind_nvfp4_weight(
    ...
    const artifact::ObjectHandle parent =
        binder.require_tensor(
            name,
            NumericFormat::NVFP4,
            artifact::StorageLayout::BlockScaleK16M128x4V1,
            shape);
    ...
    const std::uint32_t weight_bits =
        read_u32_le(
            binder.payload(parent).data,
            geometry.weight_divisor_offset,
            name);

    const std::uint32_t input_bits =
        read_u32_le(
            binder.payload(input_divisor).data,
            0,
            input_divisor_name);
...
const bool mtp_gate_up_nvfp4 =
    binder.contains(
        "mtp/layer/mlp/gate_up_projection/input_scale_divisor");

const bool mtp_down_nvfp4 =
    binder.contains(
        "mtp/layer/mlp/down_projection/input_scale_divisor");

if (mtp_gate_up_nvfp4 != mtp_down_nvfp4) {
    throw artifact::ArtifactError(
        "MTP MLP NVFP4 profile is incomplete: "
        "gate_up/down input divisors must either both exist or both be absent");
}

if (mtp_gate_up_nvfp4) {
    ...
    const bool has_vision = binder.contains("vision/patch_embedding");

    if (features.vision && !has_vision) {
        throw artifact::ArtifactError(
            "Vision was selected but the artifact has no Vision weight bundle");
    }
