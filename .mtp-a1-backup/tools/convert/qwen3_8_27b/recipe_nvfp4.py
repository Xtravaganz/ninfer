...
def _build_quantized_matrix_recipes() -> tuple[
    ...
    # MTP MLP: keep the rest of MTP on the official/W8 route, but source
    # gate/up/down from the calibrated NVFP4 checkpoint.
    mtp_prefix = "mtp.layers.0."
    mtp_object_prefix = "mtp/layer/"

    mtp_gate = _source(
        mtp_prefix + "mlp.gate_proj",
        17408,
        5120,
    )
    mtp_up = _source(
        mtp_prefix + "mlp.up_proj",
        17408,
        5120,
    )
    mtp_down = _source(
        mtp_prefix + "mlp.down_proj",
        5120,
        17408,
    )

    mtp_gate_up_sources = (mtp_gate, mtp_up)
    divisor_groups.append(mtp_gate_up_sources)

    nvfp4_weights.extend(
        (
            Nvfp4WeightRecipe(
                mtp_object_prefix + "mlp/gate_up",
                (34816, 5120),
                (_all(mtp_gate), _all(mtp_up)),
                mtp_gate_up_sources,
            ),
            Nvfp4WeightRecipe(
                mtp_object_prefix + "mlp/down",
                mtp_down.shape,
                (_all(mtp_down),),
                (mtp_down,),
            ),
...
MTP_NVFP4_OBJECT_NAMES = frozenset(
    (
